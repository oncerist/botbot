...
from settings import REDDIT_APP_ID, REDDIT_APP_SECRET, REDDIT_ENABLED_ANIME_SUBREDDITS, REDDIT_ENABLED_NSFW_SUBREDDITS
    ...
    async def random(self, ctx, subreddit: str = ""):
        async with ctx.channel.typing():
            if self.reddit:
                # start working
                nsfw_flag = False
                chosen_subreddit = REDDIT_ENABLED_ANIME_SUBREDDITS[0]
                if subreddit:
                    # should take default one
                    if subreddit in REDDIT_ENABLED_ANIME_SUBREDDITS:
